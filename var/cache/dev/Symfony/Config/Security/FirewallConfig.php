...
require_once __DIR__.\DIRECTORY_SEPARATOR.'FirewallConfig'.\DIRECTORY_SEPARATOR.'PimcoreAdminPreAuthConfig.php';
...
class FirewallConfig 
{
    ...
    public function pimcoreAdminPreAuth(array $value = []): \Symfony\Config\Security\FirewallConfig\PimcoreAdminPreAuthConfig
    {
        if (null === $this->pimcoreAdminPreAuth) {
            $this->_usedProperties['pimcoreAdminPreAuth'] = true;
            $this->pimcoreAdminPreAuth = new \Symfony\Config\Security\FirewallConfig\PimcoreAdminPreAuthConfig($value);
        } elseif (0 < \func_num_args()) {
            throw new InvalidConfigurationException('The node created by "pimcoreAdminPreAuth()" has already been initialized. You cannot pass values the second time you call pimcoreAdminPreAuth().');
        }

        return $this->pimcoreAdminPreAuth;
    }
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('pimcore_admin_pre_auth', $value)) {
            $this->_usedProperties['pimcoreAdminPreAuth'] = true;
            $this->pimcoreAdminPreAuth = new \Symfony\Config\Security\FirewallConfig\PimcoreAdminPreAuthConfig($value['pimcore_admin_pre_auth']);
            unset($value['pimcore_admin_pre_auth']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['pimcoreAdminPreAuth'])) {
            $output['pimcore_admin_pre_auth'] = $this->pimcoreAdminPreAuth->toArray();
        }
